...
git clone -q $PROJECT_URL -b ex0 $DIR_LOCAL
echo "[STATUS] Example 4: Github clone done"
pushd $DIR_LOCAL
...
git push -q -f origin official/$PROJECT_BRANCH-3:refs/heads/$PROJECT_BRANCH-3
git checkout --detach origin/$PROJECT_BRANCH-2
echo "[STATUS] Example 4: Github update done"
popd
